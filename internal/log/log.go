...
var std = New(os.Stderr, InfoLevel, defaultConfig)
...
func NewTeeWithRotate(tops []TeeOption, lc *LogConfig, opts ...Option) *Logger {
...
func New(writer io.Writer, level Level, lc *LogConfig, opts ...Option) *Logger {
	if writer == nil {
	...
	var l zapcore.Level
	if err := (&l).UnmarshalText([]byte(lc.Level)); err != nil {
...
func Init(lc *LogConfig) {
	...
	if lc.SplitErrorLog {
		tops = []TeeOption{
			{
				Filename: lc.OutputPath + "/access.log",
				Ropt:     lc.Rotate,
			...
			{
				Filename: lc.OutputPath + "/error.log",
				Ropt:     lc.Rotate,
				Lef: func(lvl Level) bool {
					return lvl > InfoLevel
				},
		...
		tops = []TeeOption{
			{
				Filename: lc.OutputPath + "/app.log",
				Ropt:     lc.Rotate,
				Lef: func(lvl Level) bool {
					return true
				},
	...
	logger := NewTeeWithRotate(tops, lc)
	ResetDefault(logger)
}
